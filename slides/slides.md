...
- Exposes agents as **OpenAI-compatible REST endpoints** — any OpenAI client talks to it
- Ships with **DevUI** — a browser-based tool for testing agents during development
...
- Any OpenAI-compatible client talks to it
- DevUI uses this endpoints
...
### Agent–User Interaction (AG-UI) Protocol `/agui/`

<v-clicks>

- Browser and apps
- Allows shared state and frontend tools
...
```mermaid {scale: 0.7}
sequenceDiagram
  DevUI->>AgentService: POST /openai/v1/chat
...
graph LR
  ...
  Agent["Agent Service\n(.NET, AGUI)"]
  LLM["Azure OpenAI"]
  Browser -->|"CopilotKit protocol\n(HTTPS + streaming)"| Runtime
  Runtime -->|"AGUI\n(streaming events)"| Agent
...
- Runtime also handles **header forwarding** from browser to agent (crucial for auth!)
...
sequenceDiagram
  ...
  Agent->>Agent: Validate JWT → User.Identity.Name
...
<<< @/snippets/server.ts#forward-headers {lines:true}
...
<<< @/snippets/jwt-bearer.cs {1-26|2-4|7-10|12-16|22-23|26}{lines:true}
