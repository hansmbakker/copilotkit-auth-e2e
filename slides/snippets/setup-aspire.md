...
dotnet add src/CopilotKitAuthE2e.AgentService/CopilotKitAuthE2e.AgentService.csproj reference src/CopilotKitAuthE2e.ServiceDefaults/CopilotKitAuthE2e.ServiceDefaults.csproj
```

> ⚠️ Also add a reference to `Microsoft.Agents.AI.DevUI` to the AppHost project as a workaround for [microsoft/agent-framework#5779](https://github.com/microsoft/agent-framework/issues/5779). Otherwise the DevUI will only return a 404 response.

## Add the following package references to the AgentService project:
```xml
  <ItemGroup>
